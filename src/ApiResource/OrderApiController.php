...
class OrderApiController extends AbstractController
{
    ...
    {
        $order = new Order();
        $user = $this->entityManager->getRepository(User::class)->findOneBy(['id' => $request->request->get('customerId')]);
        ...
        $order->setCreatedAt(new DateTime($request->request->get('date') ?? null));
        $order->setTva($request->request->get('price') ?? 0);
        $order->setUser($user);
    ...
    public function updateOrder(Request $request, int $id): JsonResponse
    {
        ...
        $order->setCreatedAt(new DateTime($request->request->get('date') ?? $order->getCreatedAt()));
        $order->setTva($request->request->get('price') ?? $order->getQuantity());
        $order->setUser($user ?? $order->getUser());
        ...
        $order->addProduct($product ?? $order->getProduct());
    ...
    private function getOrderNormalize(Order $order ): array
    {
        return [
                'id' => $order->getId(),
                'customer_id' => $order->getUser()?->getId(),
